...
function parsePath(pathValue: string | string[] | undefined): string {
  if (!pathValue) return "";
  const raw = Array.isArray(pathValue) ? pathValue.join("/") : pathValue;
  return raw.replace(/^\/+/, "");
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const path = parsePath(req.query.path as string | string[] | undefined);
  if (!path) {
    return res.status(400).json({ error: "Missing CoinGecko path" });
  }

  const upstreamUrl = `https://api.coingecko.com/api/v3/${path}${buildQuery(req)}`;
